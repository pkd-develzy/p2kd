...
export const TabAduanWarga: React.FC<TabAduanWargaProps> = ({
  ...
  const filteredAduan = useMemo(() => {
    if (!selectedAduanFilter || selectedAduanFilter === "SEMUA") return aduanList;
    return aduanList.filter((a) => a.status === selectedAduanFilter);
  }, [aduanList, selectedAduanFilter]);

  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(25);
  const startIdx = (currentPage - 1) * pageSize;
  const pagedAduan = filteredAduan.slice(startIdx, startIdx + pageSize);
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                setSelectedAduanFilter(e.target.value);
                setCurrentPage(1);
              }}
            ...
            >
              ...
              <option value="MENUNGGU">Menunggu Verifikasi ({waitingCount})</option>
              <option value="DISETUJUI">Disetujui ({approvedCount})</option>
              <option value="DITOLAK">Ditolak ({aduanList.filter((a) => a.status === "DITOLAK").length})</option>
            </select>
          </div>
      ...
      <div className="space-y-3">
        {filteredAduan.length === 0 ? (
          ...
          pagedAduan.map((a) => {
            const isProcessing = processingId === a.id || processingId === a.nomorAduan;
            return (
              <Card
                key={a.id || a.nomorAduan}
                    ...
                    {a.status === "MENUNGGU" && (
                      <div className="flex items-center gap-1.5">
                        <Button
                          ...
                          onClick={async () => {
                            const key = a.id || a.nomorAduan;
                            setProcessingId(key);
                            try {
                              await onApproveAduan(a);
                            } finally {
                              setProcessingId(null);
                            }
                          }}
                          className="text-xs bg-emerald-600 hover:bg-emerald-500 font-bold disabled:opacity-50"
                        >
                          {isProcessing ? (
                            <>
